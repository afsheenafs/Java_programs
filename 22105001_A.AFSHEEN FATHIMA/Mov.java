...
public class Mov {
    public static void main(String[] args) {
        String[] actors = {"vijay", "thrisha", "arjun"};
   for(int i=0;i<actors.length;i++)
{
System.out.println(actors[i]);
}
        Movie movie = new Movie("kanjana", "ragava lawrence", "sundarc");
        
       int r1= movie.setRatings();
        movie.recommendMovie(r1);
System.out.println("\n");
        String[] actor = {"surya", "jothika", "boomika"};
   for(int j=0;j<actor.length;j++){
System.out.println(actor[j]);
}
        Movie m1 = new Movie("mangatha", "venkat prabhu", "mehendran");
